...
/**
 *  pg_dump of databases  :
 *  msi_db ,lcms_db ,uds_db schema and some selected rows of uds_db
 *  
 */
	...
	val udsDbConnector = dsConnectorFactory.getUdsDbConnector
			...
			try {
				...
				if((projectId>0)&&(pathSource!=null)&&(!pathSource.isEmpty)&&(pathDestination!=null)&&(!pathDestination.isEmpty)){
							  ...
							  val createPgDumpSelectedRowsTables=new StringBuilder
								if((externalDbMsi.getHost()!=null) &&(!externalDbMsi.getHost().isEmpty) &&  (externalDbMsi.getDbUser()!=null) && (!externalDbMsi.getDbUser().isEmpty) &&
								   (externalDbMsi.getDbName() !=null) && (!externalDbMsi.getDbName().isEmpty) && (externalDbLcms.getDbName()!=null) && (!externalDbLcms.getDbName().isEmpty) && (externalDbMsi.getPort()>0)){ 
											try {
										       ...
										       execPgDump(externalDbMsi.getHost(),externalDbMsi.getPort(),externalDbMsi.getDbUser(),externalDbMsi.getDbPassword(),externalDbMsi.getDbName(),externalDbLcms.getDbName(),pathDestination,pathSource,projectId) 
											  	 //rows project 
                                ...
                                val fw = new FileWriter(fileToWrite.getAbsoluteFile())
                                 ...
                                 case ex: IOException => logger.error("IOException", ex)	
											  ...
											  //empty stringbuilder 
												if((createPgDumpSelectedRowsTables!=null)&&(createPgDumpSelectedRowsTables.length>0)) createPgDumpSelectedRowsTables.setLength(0)
											}
					...
					logger.error("Some parameters are missing for the project #"+projectId+" check if projectId ,source path or destination path are empty or null")
				}
