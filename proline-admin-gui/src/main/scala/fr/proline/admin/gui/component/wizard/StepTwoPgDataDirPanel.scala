 ...
 * ************************************************** *
 */
class StepTwoPgDataDirPanel(onSelectionChange: String => Boolean = null)(implicit val parentStage: Stage) extends VBox with IConfigFilesPanel with LazyLogging {
  ...
  val dataDirField = new TextField {
    ...
    if (QuickStart.postgresDataDirIsEmpty() == false) text = QuickStart.postgresqlDataDir
  ...
  def saveForm(): Unit = {
    ...
    val adminConfigFileOpt = QuickStart.getAdminConfigFile()
