    ...
    root = new VBox {
      ...
      content = List(
        ...
        new Label(wText) {
          ...
          spacing = 10
          content = List(new Label("Full path PostgreSQL data directory: "),
            new TextField {
              minWidth = 350
            })
        },
...
object SetUpPathDialog {
  def apply(
    wText: String,
    wTitle: String = "",
    wParent: Option[Stage] = Option(QuickStart.stage),
    isResizable: Boolean = false) {
    new SetUpPathDialog(wTitle, wText, wParent, isResizable).showAndWait()
  }
}
